    ...
    fs.readFile(path.join(__dirname, 'player_list_week11.csv'), 'utf8', function(err, data) {
        ...
        for (var contestant in App.Contestants) {
            if (App.Contestants.hasOwnProperty(contestant)) {
                ...
                App.ContestantsSimulated[contestant.name] = JSON.parse(JSON.stringify(contestant))
...
start().then(function() {
    ...
    server.get('/api/teams/:team', function(req, res){
        ...
        var now = new Date().getTime()
            ...
            , lastPollGameTime = (lastPoll / (minutes * 1000))
            , lastPollQuarter = Math.floor(lastPollGameTime / 15) + 1
            , lastPollQuarterTime = lastPollGameTime % 15
            , plays = []
